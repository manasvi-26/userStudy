...
function traverse(bone,joints){
	...
	for(let i=0;i<bone.children.length;i++){
        ...
        if(joints[child.name] == undefined ){
			bone.remove(child)
...
function loadModel(filePath,scene,modelName){
	...
	loader.load( filePath, function ( object ) {
		...
		traverse(object.children[0], scene.userData.joints)
        scene.add(object)
...
function get_time(){
    console.log(45 - Math.min(45, parseInt((Date.now() - clock_start_time )/1000)))
...
function render() {
    ...
    scenes.forEach( function ( scene ) {
        ...
        for(let i=0;i<4;i++){
            if( models[i] != undefined)models[i].visible = false;
        }
        // so something moves
        ...
        if(scene.userData.model == "actor"){
            ...
            for ( let person = 0; person < 1;person++){
                ...
                for (let joint=0;joint<21;joint++){
                    ...
                    let bone = joints_index_withoutFinger[joint]; 
                    scene.userData.joints[bone].rotation.x = actions[timer][person][joint][0]; 
                    scene.userData.joints[bone].rotation.y = actions[timer][person][joint][1]; 
                    scene.userData.joints[bone].rotation.z = actions[timer][person][joint][2]; 
